...
if not filtered_df.empty:
    ...
    with row1_col1:
        st.markdown('<div style="text-align: right; font-family: \'Tajawal\', sans-serif; font-size: 28px; font-weight: bold; margin-top: 40px; margin-bottom: 20px; border-right: 5px solid #1a2a3a; padding-right: 15px;">متوسط التقييم حسب النوع</div>', unsafe_allow_html=True)
        avg_rating_df = filtered_df.groupby('property_type')['rating'].mean().reset_index()
        fig1 = px.bar(avg_rating_df, x='property_type', y='rating', color='property_type', 
                      text_auto='.1f', color_discrete_sequence=pastel_colors)
        st.plotly_chart(fig1, use_container_width=True)

    with row1_col2:
        st.markdown('<div style="text-align: right; font-family: \'Tajawal\', sans-serif; font-size: 28px; font-weight: bold; margin-top: 40px; margin-bottom: 20px; border-right: 5px solid #1a2a3a; padding-right: 15px;">توزيع حصة أنواع السكن</div>', unsafe_allow_html=True)
    ...
    with row2_col2:
        st.markdown('<div style="text-align: right; font-family: \'Tajawal\', sans-serif; font-size: 28px; font-weight: bold; margin-top: 40px; margin-bottom: 20px; border-right: 5px solid #1a2a3a; padding-right: 15px;">تفاوت وتوزيع الأسعار</div>', unsafe_allow_html=True)
        fig4 = px.box(filtered_df, x='property_type', y='price', color='property_type',
                      color_discrete_sequence=pastel_colors)
